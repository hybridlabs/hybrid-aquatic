...
open class BaseWoodBlock(settings: Properties) : RotatedPillarBlock(settings) {
    ...
    override fun getToolModifiedState(
        ...
        if (context!!.itemInHand.item is AxeItem) {
            if (state!!.block == HybridAquaticWoodBlocks.DRIFTWOOD_LOG.get())
                return HybridAquaticWoodBlocks.STRIPPED_DRIFTWOOD_LOG.get().defaultBlockState()
                    .setValue(AXIS, state.getValue(AXIS))
            if (state.block == HybridAquaticWoodBlocks.DRIFTWOOD_WOOD.get())
                return HybridAquaticWoodBlocks.STRIPPED_DRIFTWOOD_WOOD.get().defaultBlockState()
                    .setValue(AXIS, state.getValue(AXIS))
        }
