...
class HybridAquaticGlobalLootModifier(
    ...
    override fun doApply(
        ...
        if (context.queriedLootTableId == target) {
            for ((idx, stack) in generatedLoot.withIndex()) {
                if (!stack.itemHolder.`is`(item_tag)) continue
                ...
                generatedLoot.addAll(
                    lootTable.getRandomItems(
                        LootParams(context.level, emptyMap(), emptyMap(), context.luck)
                    )
                )
            }
        }
    ...
    companion object {
        val CODEC: Codec<HybridAquaticGlobalLootModifier> = RecordCodecBuilder.create<HybridAquaticGlobalLootModifier> { inst ->
            @Suppress("DEPRECATION") codecStart<HybridAquaticGlobalLootModifier>(inst).and(
                inst.group(
                    ResourceLocation.CODEC.fieldOf("target").forGetter(HybridAquaticGlobalLootModifier::target),
                    TagKey<Item>.codec(Registries.ITEM).fieldOf("item_tag").forGetter(
                            HybridAquaticGlobalLootModifier::item_tag
                        ),
                    Codec.FLOAT.fieldOf("chance").forGetter(HybridAquaticGlobalLootModifier::chance),
                    WeightedRandomList.codec(WeightedEntry.Wrapper<ResourceLocation>.codec(ResourceLocation.CODEC))
                        .fieldOf("tables").forGetter(HybridAquaticGlobalLootModifier::tables)
                )
            ).apply(inst, ::HybridAquaticGlobalLootModifier)
        }
        ...
        fun registerGlobalLootModifiers() {
            ...
            lootModifiers.register<Codec<out IGlobalLootModifier>>("ha_loot_modifier", HybridAquaticGlobalLootModifier::CODEC)
        }
    }
}