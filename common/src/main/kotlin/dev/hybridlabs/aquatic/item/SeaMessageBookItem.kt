...
SeaMessageBookItem(settings: Properties) : Item(settings) {
    ...
    override fun use(world: Level, player: Player, hand: InteractionHand): InteractionResultHolder<ItemStack> {
        ...
        if (!stack.has(DataComponents.WRITTEN_BOOK_CONTENT))
            setWrittenBookContent(stack, world.registryAccess())
    ...
    companion object {
        ...
        private fun setWrittenBookContent(
            stack: ItemStack,
        ...
        fun getSeaMessage(stack: ItemStack, registryManager: HolderLookup.Provider): SeaMessage? {
            ...
            val registry = registryManager.lookup(HARegistryKeys.SEA_MESSAGE).get()
            return registry.get(ResourceKey.create(HARegistryKeys.SEA_MESSAGE, id)).getOrNull()?.value()
        }
    }
}
