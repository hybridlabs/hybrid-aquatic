...
class HerbivoreGrazeGoal(
    ...
    override fun canUse(): Boolean {
        ...
        val found = findNearbySeagrassBlock()
    ...
    override fun canContinueToUse(): Boolean {
        ...
        return grazeTime > 0 &&
                waterAnimal.level().getBlockState(pos).`is`(HABlockTags.HERBIVORE_EDIBLE)
    }
    ...
    private fun findNearbySeagrassBlock(): BlockPos? {
        ...
        for (x in -radius..radius) {
            for (y in -2..2) {
                for (z in -radius..radius) {
                    ...
                    if (
                        level.getBlockState(pos).`is`(HABlockTags.HERBIVORE_EDIBLE) &&
