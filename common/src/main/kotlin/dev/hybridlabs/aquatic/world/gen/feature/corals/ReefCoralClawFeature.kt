...
class ReefCoralClawFeature(codec: Codec<NoneFeatureConfiguration>) : ReefCoralFeature(codec) {
    override fun placeFeature(
        ...
        if (!this.placeCoralBlock(level, random, pos, state)) {
            ...
            for (direction1 in list.subList(0, i)) {
                ...
                var l = 0
                while (l < j && this.placeCoralBlock(level, random, mutableBlockPos, state)) {
