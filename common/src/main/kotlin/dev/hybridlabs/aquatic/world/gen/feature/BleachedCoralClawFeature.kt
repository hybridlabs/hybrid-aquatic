...
class BleachedCoralClawFeature(codec: Codec<NoneFeatureConfiguration?>) : BleachedCoralFeature(codec) {
    override fun placeFeature(
        ...
        if (!this.placeBleachedCoralBlock(level, random, pos, state)) {
            return false
        } else {
            ...
            for (direction1 in list.subList(0, i)) {
                ...
                var l = 0
                while (l < j && this.placeBleachedCoralBlock(level, random, mutableBlockPos, state)) {
                ...
                for (i1 in 0..<k) {
                    ...
                    if (!this.placeBleachedCoralBlock(level, random, mutableBlockPos, state)) {
                        break
                    }
