...
class DelesseriaFeature(codec: Codec<DelesseriaFeatureConfig>) :
    Feature<DelesseriaFeatureConfig>(codec) {
    override fun generate(context: FeatureContext<DelesseriaFeatureConfig>): Boolean {
        var i = 0
        ...
        if (structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER)) {
            val blockState = HybridAquaticBlocks.DELESSERIA.defaultState
            val blockState2 = HybridAquaticBlocks.DELESSERIA_PLANT.defaultState
            ...
            for (l in 0..k) {
                if (structureWorldAccess.getBlockState(blockPos2)
                    ...
                    if (blockState.canPlaceAt(structureWorldAccess, blockPos3) && !structureWorldAccess.getBlockState(
                            blockPos3.down()
                        ).isOf(HybridAquaticBlocks.DELESSERIA)
                    ) {
                        structureWorldAccess.setBlockState(
                            blockPos3,
                            blockState.with(KelpBlock.AGE, random.nextInt(4) + 20) as BlockState,
