...
class SugarKelpFeature(codec: Codec<SugarKelpFeatureConfig>) :
    Feature<SugarKelpFeatureConfig>(codec) {
    override fun generate(context: FeatureContext<SugarKelpFeatureConfig>): Boolean {
        var i = 0
        ...
        if (structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER)) {
            val blockState = HybridAquaticBlocks.SUGAR_KELP.defaultState
            val blockState2 = HybridAquaticBlocks.SUGAR_KELP_PLANT.defaultState
            ...
            for (l in 0..k) {
                if (structureWorldAccess.getBlockState(blockPos2)
                    ...
                    if (blockState.canPlaceAt(structureWorldAccess, blockPos3) && !structureWorldAccess.getBlockState(
                            blockPos3.down()
                        ).isOf(HybridAquaticBlocks.SUGAR_KELP)
                    ) {
                        structureWorldAccess.setBlockState(
                            blockPos3,
                            blockState.with(KelpBlock.AGE, random.nextInt(4) + 20) as BlockState,
