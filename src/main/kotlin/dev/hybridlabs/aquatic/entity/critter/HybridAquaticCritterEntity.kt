...
open class HybridAquaticCritterEntity(type: EntityType<out HybridAquaticCritterEntity>, world: World) : WaterCreatureEntity(type, world), GeoEntity {
    ...
    override fun initGoals() {
        ...
        goalSelector.add(2, WalkToRandomPlaceGoal(this, 0.50, 6))
    ...
    override fun tick() {
        ...
        if (isAiDisabled) {
            ...
            if (world.isClient && isOnGround && isAttacking) {
    ...
    internal class FishMoveControl(private val fish: HybridAquaticCritterEntity) : MoveControl(fish) {
        override fun tick() {
            if (fish.isOnGround) {
                fish.velocity = fish.velocity.add(0.0, -0.1, 0.0)
            }
            if (state == State.MOVE_TO && !fish.getNavigation().isIdle) {
                ...
                if (e != 0.0) {
                    ...
                    fish.velocity = fish.velocity.add(0.0, 0.0, 0.0)
                }
                if (d != 0.0 || g != 0.0) {
                ...
                fish.movementSpeed = 0.1f
            }
        }
    }

    internal class WalkToRandomPlaceGoal(private val fish: HybridAquaticCritterEntity, d: Double, i: Int) :
        WanderAroundGoal(fish, 1.0, 40) {
    ...
    companion object {
        val MOISTNESS: TrackedData<Int> = DataTracker.registerData(HybridAquaticCritterEntity::class.java, TrackedDataHandlerRegistry.INTEGER)
