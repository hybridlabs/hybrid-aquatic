...
public class HACustomTrades {
    public static void registerCustomTrades() {
        ...
        TradeOfferHelper.registerVillagerOffers(VillagerProfession.FISHERMAN, 3, factories -> {
            factories.add((entity, random) -> new MerchantOffer(new ItemCost(HAItems.INSTANCE.getPEARL().get(), 1), new ItemStack(Items.EMERALD, 2), 6, 15, 0.05f));
            factories.add((entity, random) -> new MerchantOffer(new ItemCost(HAItems.INSTANCE.getBLACK_PEARL().get(), 1), new ItemStack(Items.EMERALD, 5), 3, 15, 0.05f));
        });
    }
}
